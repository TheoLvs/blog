...
Like many optimization problems, it's a NP-hard problem, which in short means that it's easy (in terms of speed) to solve for 10 cities, a little harder to solve for 50. And it would take a while for 50,000 cities. Because it's impossible to brute force, there are typical optimization techniques you can use to solve the TSP: like ???. 
...
In Python, the easiest way to get started with TSP and its variants is probably the great open source library OR-Tools by Google. And if you want to learn more about discrete optimization, I can only recommend the great MOOC by ??? you can find here. 
...
- The gamma factor ``gamma`` is the "lookahead" factor, and control long-term planning. Indeed if gamma = 0, you will only get the next action reward, if gamma = 1 you will be more oriented towards the future rewards. 
...
## TODO
- Meilleure explication du Q Learning
- Un jour sans fin et Doctor Strange
